...
const CourseSelection = ({course}) => {
 ...
 function SelectCourse(key){
  ...
  return (
    ...
    <div key={key} onClick={()=>SelectCourse(key)} className={`flex items-center ${(selectedKey===key)&&'bg-red-800 text-white'} font-bold border-red-800 justify-center lg:rounded-3xl w-16 lg:w-36 border py-2 lg:py-3`}>
      <h1 >{key}</h1>  
    </div>
  ))}
    </div>
 <div className='lg:h-full h-[30vh] w-[100%] lg:w-[60%] overflow-y-auto  rounded-xl border shadow-md'>
...
const OfferingCollegesLIst=({college})=>{
  return(
    <div className='flex p-2 pt-7 flex-wrap gap-3 justify-center' > 
      {college?.map((college)=>(<div className='flex border w-32 lg:w-40 rounded-lg text-nowrap overflow-hidden flex-col  justify-center p-2 border-red-800'>
        <FaUniversity className='m-auto text-2xl lg:text-4xl'/>
        <p className='text-ellipsis w-[100%] overflow-hidden whitespace-nowrap'>{college}</p></div>))}
    </div>
  )
}